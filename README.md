...
- **Authorization includes unrelated Google permissions:** remove the existing Google Account authorization and authorize again. The integration deliberately requests a Health-only token because Google Health rejects tokens containing disallowed scopes.
